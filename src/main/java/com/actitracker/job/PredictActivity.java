...
public class PredictActivity {
    ...
    // method loads the model from folder structure and predict the result (user activity)
    // returns class id (0-5), as total number of classes are 6.
    public static double predict(JavaSparkContext sc) {

        // load already created model from respected folder for the model
        DecisionTreeModel model = DecisionTreeModel.load(sc.sc(), "actitracker/decision_tree/");
